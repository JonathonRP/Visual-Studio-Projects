...
namespace TestBrokenApp.ViewModels
{
    public class TasksViewModel : BaseViewModel
    {
            ...
            : base (navigationService, eventAggregator, pageDialogService)
        {
            ...
            {
                var _task = Tasks.Where(x => x.Id == task.Task.Id).FirstOrDefault();
                Tasks.Remove(_task);
                Tasks.Add(task.Task);
                await DataStore.UpdateAsync(task.Task);
            });
        ...
        private bool CanNavigate(ToDoTask task)
        {
            ...
            if (task.InEdit == true)
            {
                ...
                EventAggregator.GetEvent<UpdateTask>().Publish(new UpdateTask() { Task = task });
                LoadItemsCommand.Execute(null);
                return false;
            }
